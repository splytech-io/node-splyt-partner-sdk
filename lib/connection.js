...
const CoEventEmitter = require('@splytech-io/splyt-ws-connection/lib/co-event-emitter');
...
module.exports = class SplytPartnerSDKConnection extends CoEventEmitter {
  ...
  _handlePush(method, data) {
    ...
    if (!handler) {
      const message = `Unhandled push message received '${method}'`;
  ...
  _handleRequest(method, data) {
    ...
    if (!handler) {
      const message = `Unhandled request message received '${method}'`;
